...
async def run_whatsapp_bridge_event_worker_loop(
    *,
    ...
    worker = WhatsAppBridgeEventWorker(
    ...
    try:
        while True:
            async with AsyncSessionLocal() as session:
                connections = await repository.list_active_whatsapp_connections(session)
            await worker.reconcile(connections)
