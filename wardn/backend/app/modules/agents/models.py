...
class AgentRun(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    previous_agent_run_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_runs.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
...
class AgentRunResumeJob(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "agent_run_resume_jobs"
    __table_args__ = (
        CheckConstraint(
    ...
    approval_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_tool_approvals.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
