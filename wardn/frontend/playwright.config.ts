...
export default defineConfig({
  ...
  webServer: [
    {
      command: `WARDN_E2E_BACKEND_PORT=${mockBackendPort} node e2e/mock-backend.mjs`,
      url: `${mockBackendUrl}/__test/health`,
      reuseExistingServer: !process.env.CI,
      timeout: 30_000,
    },
    {
      command: `HOSTNAME=127.0.0.1 PORT=${frontendPort} WARDN_BACKEND_URL=${mockBackendUrl} node .next/standalone/wardn/frontend/server.js`,
