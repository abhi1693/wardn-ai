...
export function AppShellError({ embedded = false, error, reset, scope }: AppShellErrorProps) {
  useEffect(() => {
    console.error(error);
  }, [error]);
  ...
  const content = (
    <section
      className={
        embedded
          ? "mx-auto flex min-h-screen max-w-2xl items-center px-6 py-20"
          : "mx-auto flex min-h-[calc(100vh-3.5rem)] max-w-2xl items-center px-6 py-12"
      }
    >
